...
namespace _5._2.transformations_exercise2
{
    public partial class Form1 : Form
    {
        ...
        private void OpenGLControl1_OpenGLDraw(object sender, SharpGL.RenderEventArgs args)
        {
            ...
            //平移
            transform = glm.translate(transform, new vec3(-0.5f, 0.5f, 0.0f));
            var scaleAmount = (float)Math.Sin((DateTime.Now - startTime).TotalSeconds);
            //缩放
            transform = glm.scale(transform, new vec3(scaleAmount, scaleAmount, scaleAmount));
            //传递给着色器
            shaderProgram.SetUniformMatrix4(GL, "transform", transform.to_array());
            //绘制第二个物体
            GL.DrawElements(OpenGL.GL_TRIANGLES, indices.Length, indices);
        ...
        private void OpenGLControl1_OpenGLInitialized(object sender, EventArgs e)
        {
            ...
            using (StreamReader sr = new StreamReader("5.2.transform.vs"))
            {
                vertexShaderSource = sr.ReadToEnd();
            }

            using (StreamReader sr = new StreamReader("5.2.transform.fs"))
            {
